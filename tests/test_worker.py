...
from .fixtures import Worker, MockRedisDemo, EXAMPLE_FILE, WorkerQuit, FailedActor, WorkerFailedActor
...
async def test_run_quit(tmpworkdir, redis_conn, demo, logcap):
    ...
    assert '1 pending tasks, waiting for one to finish before creating task for Demo.save_slow(2, 0.1)' in logcap


async def test_task_exc(redis_conn, loop, logcap):
    logcap.set_level(logging.DEBUG)

    actor = FailedActor(loop=loop)

    await actor.add_numbers(1, 2)
    await actor.close()

    worker = WorkerFailedActor(loop=loop)
